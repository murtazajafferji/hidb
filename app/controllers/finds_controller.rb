  ...
  def index
    if params[:user]
  	...
  	if params[:word]
  	  ...
  	  @results = Internship.find(:all, :conditions => ['LOWER(semester) LIKE ? or year LIKE ? or LOWER(course) LIKE ? or LOWER(industry) LIKE ? or LOWER(company_name) LIKE ? or LOWER(company_department) LIKE ? or LOWER(city) LIKE ? or LOWER(state) LIKE ? or LOWER(country) LIKE ?', "%#{$query.downcase}%", "%#{$query.downcase}%", "%#{$query.downcase}%", "%#{$query.downcase}%", "%#{$query.downcase}%", "%#{$query.downcase}%", "%#{$query.downcase}%", "%#{$query.downcase}%", "%#{$query.downcase}%"]).to_a
  	  @results += User.find(:all, :conditions => ['LOWER(email) LIKE ? or LOWER(first_name) LIKE ? or LOWER(last_name) LIKE ? or LOWER(major) LIKE ? or LOWER(minor) LIKE ? or yog LIKE ?', "%#{$query.downcase}%", "%#{$query.downcase}%", "%#{$query.downcase}%", "%#{$query.downcase}%", "%#{$query.downcase}%", "%#{$query.downcase}%"]).to_a
